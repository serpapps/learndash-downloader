...
### 7.5 Playwright/Puppeteer Browser Automation

#### 7.5.1 Playwright Video Extraction
```javascript
const { chromium } = require('playwright');

async function extractLoomVideo(url) {
...
#### 7.5.2 Puppeteer Implementation
```javascript
const puppeteer = require('puppeteer');

async function getLoomVideoUrls(shareUrl) {
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
    
    const requests = [];
    page.on('response', response => {
        if (response.url().includes('cdn.loom.com') && 
...
#### 9.1.3 Geo-blocking and Access Restrictions
```bash
...
test_with_proxy() {
    ...
    for proxy in "${proxy_list[@]}"; do
        ...
        if yt-dlp $proxy --dump-json "$url" > /dev/null 2>&1; then
...
download_with_proxy() {
    ...
    if [ -n "$proxy_server" ]; then
        yt-dlp --proxy "$proxy_server" -o "$output_dir/%(title)s.%(ext)s" "$url"
    else
...
diagnose_hls_stream() {
    ...
    echo "Diagnosing HLS stream: $hls_url"
    
    # Check if master playlist is accessible
    curl -I "$hls_url"
    
    # Download and inspect master playlist
    curl -s "$hls_url" > master.m3u8
    echo "Master playlist contents:"
    cat master.m3u8
    
    # Test individual quality streams
    grep -oE "https://[^\"]*\.m3u8" master.m3u8 | while read quality_url; do
        echo "Testing quality stream: $quality_url"
        curl -I "$quality_url"
    done
}

# Download HLS with segment retry
ffmpeg_hls_download() {
    local hls_url="$1"
    local output_file="$2"
    
    # Download with segment retry and error handling
...
# Handle broken HLS segments
repair_hls_download() {
    local hls_url="$1"
    local output_file="$2"
    
    # Download with error tolerance
...
convert_for_compatibility() {
    ...
    # Convert to H.264 baseline profile for maximum compatibility
    ffmpeg -i "$input_file" \
           -c:v libx264 \
           -profile:v baseline \
           -level 3.0 \
...
fix_codec_issues() {
    ...
    # Re-encode with standard codecs
    ffmpeg -i "$input_file" \
           -c:v libx264 \
           -c:a aac \
           -avoid_negative_ts make_zero \
...
test_cdn_speed() {
    local video_id="$1"
    local test_url="https://cdn.loom.com/sessions/$video_id/transcoded/mp4/720/video.mp4"
    
    echo "Testing download speed to Loom CDN..."
    
    # Test with curl
    curl -w "Connect: %{time_connect}s, Download: %{time_total}s, Speed: %{speed_download} bytes/s\n" \
         -o /dev/null \
         -s \
         "$test_url"
    ...
    # Test first 1MB
    curl -r 0-1048576 \
         -w "First 1MB - Time: %{time_total}s, Speed: %{speed_download} bytes/s\n" \
         -o /dev/null \
         -s \
         "$test_url"
}

# Optimize download settings based on connection
optimize_download_settings() {
    local url="$1"
    local speed_test_result
    
    # Test download speed
    speed_test_result=$(curl -w "%{speed_download}" -o /dev/null -s "$url" | head -c 10)
    
    if (( $(echo "$speed_test_result < 1000000" | bc -l) )); then
        echo "Slow connection detected (<1MB/s)"
        ...
        yt-dlp --limit-rate 500K \
               ...
               --sleep-requests 1 \
               "$url"
    elif (( $(echo "$speed_test_result < 5000000" | bc -l) )); then
        echo "Medium speed connection (1-5MB/s)"
        echo "Using moderate settings..."
        yt-dlp --limit-rate 2M \
               --retries 3 \
               "$url"
    else
        echo "Fast connection detected (>5MB/s)"
        echo "Using aggressive settings..."
...
```bash
# Download with memory constraints
...
monitor_memory_usage() {
    ...
    while kill -0 "$download_pid" 2>/dev/null; do
        ps -p "$download_pid" -o pid,vsz,rss,pcpu,pmem,comm
        sleep 5
    done
}
```
...
verify_video_integrity() {
    ...
    if ffprobe -v error -select_streams v:0 -show_entries stream=codec_name -of csv=p=0 "$video_file" > /dev/null 2>&1; then
        echo "✓ Video structure is valid"
...
verify_file_checksum() {
    ...
    if [ -n "$expected_hash" ]; then
        ...
        echo "No expected hash provided, generating checksum:"
        sha256sum "$file_path"
    fi
}
```
...
```bash
# Attempt to repair corrupted MP4 files
repair_corrupted_mp4() {
    local input_file="$1"
    local output_file="${2:-${input_file%.mp4}_repaired.mp4}"
    
    echo "Attempting to repair: $input_file"
    
    # Try to fix with ffmpeg
    if ffmpeg -err_detect ignore_err \
              -i "$input_file" \
              -c copy \
              -avoid_negative_ts make_zero \
              "$output_file"; then
        ...
        if verify_video_integrity "$output_file"; then
            echo "✓ Repaired file is valid"
            return 0
        else
            echo "✗ Repair failed - file still corrupted"
            return 1
        fi
    else
...
# Fix timestamp and seeking issues
fix_video_timestamps() {
    local input_file="$1"
    local output_file="${2:-${input_file%.mp4}_fixed.mp4}"
    
    # Fix timestamp issues and add fast start for web playback
    ffmpeg -i "$input_file" \
           -c copy \
           -avoid_negative_ts make_zero \
           -movflags +faststart \
...
# Reconstruct video index for seeking
rebuild_video_index() {
    local input_file="$1"
    local output_file="${2:-${input_file%.mp4}_indexed.mp4}"
    
    # Rebuild index for proper seeking
    ffmpeg -i "$input_file" \
           -c copy \
           -movflags +faststart \
           "$output_file"
...
- **Playwright/Puppeteer**: Browser automation for complex cases
